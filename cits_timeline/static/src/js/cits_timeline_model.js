    ...
    return AbstractModel.extend({
        ...
        timelineItemToRecord: function (item) {
            ...
            if (this.mapping.date_stop) {
                record[this.mapping.date_stop] = end;
            }
        ...
        reload: function (handle, params) {
            if (params.domain || params.groupBy) {
                ...
                return this._loadItems([['id', '=', params.updateId]], group);
            } else {
                return this._loadRange({
                    start: this.data.start_date,
                    end: this.data.end_date
        ...
        updateItem: function (item) {
            ...
            for (var k in record) {
                if (record[k] && record[k]._isAMomentObject) {
                    context[k] = moment(context[k]).utcOffset('-02:00').format('YYYY-MM-DD HH:mm:ss');
//                    context[k] = dateToServer(context[k])
        ...
        _getRangeDomain: function (range) {
            var domain = [[this.mapping.date_start, '<=', dateToServer(range.end)]];
            if (this.mapping.date_stop) {
                domain.push([this.mapping.date_stop, '>=', dateToServer(range.start)]);
            } else if (!this.mapping.date_delay) {
                domain.push([this.mapping.date_start, '>=', dateToServer(range.start)]);
            }
            return domain;
        },
        ...
        _loadItems: function (domain, group) {
            ...
            return self._rpc({
                ...
                _.each(records, function (record) {
                    ...
                    let className = '';
                    if (self.fieldColor) {
                        let colorValue = record[self.fieldColor];
        ...
        _getRecordRange: function (record) {
            var date_start;
            var date_end;
            var date_delay = record[this.mapping.date_delay] || 1.0;
